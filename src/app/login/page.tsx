...
export default function LoginPage() {
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    if (otpCode.length !== 6 || isNaN(Number(otpCode))) {
      setErrorMessage('6자리 숫자를 정확히 입력하세요.')
      return
    }
    setIsPending(true)
    setErrorMessage(null)
    ...
    try {
      const res = await verify2faLoginAction(tempToken, otpCode)
      if (res.error) {
        setErrorMessage(res.error)
      ...
      setErrorMessage('2차 인증 대조 중 통신 에러가 발생했습니다.')
    } finally {
      setIsPending(false)
    }
  }
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center bg-[#fafafa] dark:bg-[#070708] overflow-hidden px-6 py-20 transition-colors duration-500">
      ...
      <div 
        className="w-full max-w-[460px] bg-white/90 dark:bg-neutral-900/80 border border-neutral-200/60 dark:border-neutral-800/60 backdrop-blur-xl rounded-3xl p-8 sm:p-10 shadow-[0_32px_120px_rgba(0,0,0,0.06)] dark:shadow-[0_32px_120px_rgba(0,0,0,0.35)] relative z-10 transition-all duration-500"
      >
        {!requires2FA ? (
          ...
          /* 🔐 2FA OTP 입력 인터페이스 */
          <form onSubmit={handleOtpSubmit} className="flex flex-col gap-5 animate-in fade-in slide-in-from-right-8 duration-500">
            <div className="flex flex-col items-center text-center select-none">
              <div className="w-12 h-12 bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-full flex items-center justify-center mb-4 shadow-inner">
              ...
              <h2 className="text-xl font-extrabold text-neutral-900 dark:text-white mb-2 select-none">
                2단계 보안인증 (2FA)
              </h2>
              <p className="text-xs text-neutral-400 dark:text-neutral-500 max-w-sm leading-relaxed select-none">
                계정 보안 강화를 위해 Google Authenticator 앱에서 실시간으로 생성되는 6자리 보안 인증 번호를 입력해 주세요.
              </p>
            </div>

            <div>
              <label className="block text-[10px] uppercase font-bold text-neutral-400 dark:text-neutral-500 mb-1.5 tracking-wider text-center" htmlFor="otpCode">
                보안 번호 6자리
              </label>
              <input
                id="otpCode"
                ...
                required
                autoFocus
                className="w-full rounded-2xl px-4 py-3.5 bg-neutral-50/50 dark:bg-neutral-950/40 border border-neutral-200 dark:border-neutral-800/80 focus:border-neutral-900 dark:focus:border-white focus:ring-4 focus:ring-neutral-900/5 dark:focus:ring-white/5 outline-none text-center text-2xl font-bold tracking-widest text-neutral-950 dark:text-white placeholder-neutral-400 transition-all duration-300 shadow-sm"
              />
            </div>
            ...
            <div className="flex gap-3 mt-3">
              <button
                ...
                className="flex-1 bg-neutral-950 dark:bg-white text-white dark:text-neutral-950 rounded-2xl py-4 font-bold text-sm hover:opacity-90 active:scale-[0.99] transition-all duration-300 shadow-md flex items-center justify-center gap-2 select-none disabled:opacity-50"
              >
                {isPending ? (
                  <div className="animate-spin rounded-full h-4 w-4 border-2 border-current border-t-transparent"></div>
                ) : (
              ...
              <button
                ...
                className="flex-1 bg-white dark:bg-neutral-900 text-neutral-600 dark:text-neutral-400 border border-neutral-200 dark:border-neutral-800 rounded-2xl py-4 font-bold text-sm hover:bg-neutral-50 dark:hover:bg-neutral-950 active:scale-[0.99] transition-all duration-300 shadow-sm text-center"
              >
                취소
              </button>
            </div>
            ...
            {errorMessage && (
              <div className="mt-2 p-4 text-xs font-semibold text-rose-600 dark:text-rose-400 bg-rose-50/50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/40 rounded-2xl text-center shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
                {errorMessage}
              </div>
            )}
          </form>
