...
import DraggablePeersBadge from '@/components/creator/DraggablePeersBadge'
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      title: `${nickname} | BlockCanvas Portfolio`,
      description: aboutText,
      url: `http://${creator_name}.craftopia.work`,
...
export default async function CreatorPortfolioPage({
    ...
    if (isOwner && !hasPortfolio) {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-neutral-50 flex-col gap-4">
          ...
          <Link href={`/dashboard/portfolio`} className="px-6 py-3 bg-black text-white rounded-lg font-bold hover:bg-neutral-800 transition-colors">
            대시보드로 가기
          </Link>
        </div>
      )
  ...
  let snsSettings = { discord: true, twitter: true, youtube: true, instagram: true, patreon: false, recommended_creators: [] }
  ...
  // Fetch only explicitly recommended profiles
  const recommendedUsernames = (snsSettings as any).recommended_creators || []
  let recommendedCreators: any[] = []

  if (recommendedUsernames.length > 0) {
    const recommendedProfiles = await prisma.profile.findMany({
      where: {
        creator_name: { in: recommendedUsernames }
      },
      select: {
        creator_name: true,
        display_name: true,
    ...
    // Sort to match the specified order in recommendedUsernames
    recommendedCreators = recommendedUsernames
      .map((username: string) => {
        const p = recommendedProfiles.find(profile => profile.creator_name === username)
        if (!p) return null
        return {
          name: p.display_name || p.creator_name,
    ...
    <ScrollProgressProvider global>
      ...
      <header className="hero-container sticky top-0 w-full flex flex-col text-white group pt-20 z-0 min-h-[600px] md:min-h-[750px] overflow-hidden">
          ...
          <div className="absolute top-0 left-0 w-full h-[450px] md:h-[600px] overflow-hidden z-0" style={{ perspective: '1000px', WebkitMaskImage: 'linear-gradient(to top, transparent, black 15%)', maskImage: 'linear-gradient(to top, transparent, black 15%)' }}>
            <img 
              src={profileData.banner_url || '/default_banner.png'} 
              alt="Banner" 
        ...
        <div className="relative z-20 text-center flex flex-col items-center mt-[250px] md:mt-[400px] w-full px-4 pb-16 pointer-events-auto">
          ...
          {/* Avatar Area */}
          <div className="hero-avatar mb-6 -mt-16 md:-mt-20 flex flex-col sm:flex-row items-center gap-6 relative justify-center">
          ...
          <div className="hero-subtitle flex items-center gap-4 mb-6">
