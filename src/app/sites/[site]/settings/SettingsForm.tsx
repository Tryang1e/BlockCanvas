...
export default function SettingsForm({ profile, allCreators = [] }: { profile: any, allCreators?: any[] }) {
  ...
  return (
    <div className="p-8 sm:p-12 space-y-12">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <select name="theme_bg_effect" defaultValue={profile.theme_bg_effect ? profile.theme_bg_effect.split('|')[0] : 'none'} className="w-full border border-neutral-300 p-4 rounded-lg bg-neutral-50 focus:bg-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all font-medium text-sm">
      ...
      <section className="bg-neutral-50/50 p-6 rounded-xl border border-neutral-200">
        <h2 className="text-xl font-bold border-b border-neutral-100 pb-4 mb-4 flex items-center justify-between">
          <span>🤝 추천 동료 크리에이터 설정</span>
          <span className="text-xs bg-black text-white px-2.5 py-1 rounded-full font-mono">Peer Recommendations</span>
        </h2>
        <p className="text-sm text-neutral-500 mb-6">
          내 포트폴리오 메인 프로필 하단에 표시할 동료 크리에이터를 추천할 수 있습니다. 
          상호 추천을 통해 다른 크리에이터의 공간으로 방문자를 유도해보세요!
        </p>
          ...
          {recommendedCreators.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-3">
              {recommendedCreators.map(username => {
                ...
                return (
                  <div key={username} className="flex items-center gap-2 bg-white border border-neutral-200 hover:border-neutral-300 px-3.5 py-2 rounded-full shadow-sm animate-in zoom-in-95 duration-200 group">
                    <div className="relative w-6 h-6 rounded-full overflow-hidden bg-neutral-200 flex-shrink-0">
                      <img src={creator.avatar_url || '/default_avatar.png'} alt={creator.display_name} className="object-cover w-full h-full" />
                    </div>
                    <div className="flex flex-col text-left">
                      <span className="text-xs font-bold text-neutral-800 leading-tight">{creator.display_name}</span>
                      <span className="text-[9px] text-neutral-400 font-mono leading-none">@{creator.creator_name}</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => removeRecommendedCreator(username)}
                      className="ml-1 w-4 h-4 rounded-full bg-neutral-100 hover:bg-red-500 hover:text-white flex items-center justify-center text-[10px] transition-colors"
                      title="추천 취소"
                    >
                      ✕
                    </button>
